package com.futurebytedance.day11.dynamicproxy

import java.lang.reflect.Proxy

class MatchService {
  //创建了一个Person
  val tom: PersonBean = getPersonInfo("tom", "男", "爱好编程")

  //得到一个给自己调用的代理对象，它替代被调用的对象
  val OwnerProxy: PersonBean = getOwnerProxy(tom)
  ...
  val mary: PersonBean = getPersonInfo("mary", "女", "爱好购物...")

  //返回一个其他用户调用的代理对象
  val nonOwnerProxy: PersonBean = getNonOwnerProxy(mary)
