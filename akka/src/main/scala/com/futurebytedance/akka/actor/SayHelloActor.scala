package com.futurebytedance.akka.actor
 ...
 */
//说明
//1.当我们继承了Actor，就是一个Actor，核心方法Receive方法重写
class SayHelloActor extends Actor {
  //说明
  //1.receive方法，会被该Actor的MailBox(实现了Runnable接口)调用
  //2.当该Actor的MailBox接收到消息，就会调用 receive
  //3.type Receive = PartialFunction[Any,Int]
...
object SayHelloActor {
  ...
  //说明
  //(1)Props[SayHelloActor] 创建了一个sayHelloActor的实例，使用反射
